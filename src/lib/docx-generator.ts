...
const createSection = (title: string, isSidebar: boolean = false) => {
  ...
  return new Paragraph({
    children: [new TextRun({ text: title, bold: true, allCaps: true, color: titleColor, size: 16, font: { name: FONT_FAMILY } })],
    border: { bottom: { color: borderColor, space: 1, value: 'single', size: 6 } },
    spacing: { after: 140, before: 280 },
  });
}
...
export function createResumeDocx(resumeData: ResumeData): Document {
  ...
  const leftColumnContent = [
    new Paragraph({
      children: [new TextRun({ text: name.toUpperCase(), bold: true, color: ACCENT_TEXT_COLOR, size: 40, font: { name: FONT_FAMILY } })],
      spacing: { after: 300 },
    }),
    ...
    ...(projects.length > 0 ? [
      ...
      ...projects.flatMap(proj => [
        new Paragraph({
          children: [new TextRun({ text: proj.title, bold: true, color: ACCENT_TEXT_COLOR, size: 18, font: { name: FONT_FAMILY } })],
    ...
    ...(keyAchievements.length > 0 ? [
      ...
      ...keyAchievements.flatMap(ach => [
        new Paragraph({
          children: [
            new TextRun({ text: ach.title, bold: true, color: ACCENT_TEXT_COLOR, size: 18, font: { name: FONT_FAMILY } })
          ],
          bullet: { level: 0 },
    ...
    ...(training.length > 0 ? [
      ...
      ...training.flatMap(course => [
        new Paragraph({
          children: [new TextRun({ text: course.title, bold: true, color: ACCENT_TEXT_COLOR, size: 18, font: { name: FONT_FAMILY } })],
          spacing: { after: 40, before: 80 },
        }),
        new Paragraph({
  ...
  const rightColumnContent = [
    new Paragraph({
      children: [new TextRun({ text: title, bold: true, color: TEXT_COLOR, size: 24, font: { name: FONT_FAMILY } })],
      spacing: { after: 80 },
    }),
    new Paragraph({
      children: [
        new TextRun({ text: [contact.phone, contact.email, contact.linkedin, contact.location, contact.extraField].filter(Boolean).join(' | '), color: LIGHT_TEXT_COLOR, size: 16, font: { name: FONT_FAMILY } }),
      ],
      spacing: { after: 200 },
    }),
    ...
    ...(summary ? [
      ...
      new Paragraph({ children: [new TextRun({ text: summary, size: 16, color: TEXT_COLOR, font: { name: FONT_FAMILY } })], spacing: { after: 100, before: 80 } }),
    ] : []),
    ...
    ...(experience.length > 0 ? [
      ...
      ...experience.flatMap(exp => [
        ...
        new Paragraph({
          children: [
            new TextRun({ text: exp.company, bold: true, color: LINK_COLOR, size: 18, font: { name: FONT_FAMILY } }),
    ...
    ...(education.length > 0 ? [
      ...
      ...education.flatMap(edu => [
        ...
        new Paragraph({
          children: [
            new TextRun({ text: edu.school, bold: true, color: LINK_COLOR, size: 18, font: { name: FONT_FAMILY } }),
